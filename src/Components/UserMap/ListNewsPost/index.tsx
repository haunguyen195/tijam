...
const ListNewsPost: FC<any> = forwardRef((props, ref) => {
  ...
  const deletePost = useCallback(async () => {
    ...
    await PostServices.deletePost(itemDeleted).then(() => {
      ShowToast(
        "success",
        language.DELETED,
        language.POST_HAVE_BEEN_REMOVE
      );

      let residualPosts = posts.filter((item) => item.id !== itemDeleted.id);
  ...
  const renderItem = useCallback(({ item }) => <Item expandAnimated={expandAnimated} color={color}
                                                     ...
                                                     showImage={showImage} confirmDeletePost={confirmDeletePost}
                                                     expand={expand} styles={styles} language={language}/>, [expand, language]);
